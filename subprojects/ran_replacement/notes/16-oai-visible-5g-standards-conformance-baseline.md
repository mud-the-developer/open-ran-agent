...
3. `F1-U / GTP-U` standards-tightening lane
   ...
   - explicit forwarding-cleanup semantics for failed cutover, stale tunnel cleanup, and bounded same-UE next-session review
   - compare fixtures that separate ping proof from user-plane subset conformance while keeping multi-session language bounded
