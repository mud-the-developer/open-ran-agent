...
| `GTP-U` tunnel rebind | Rebind the declared tunnel without claiming broad mobility or multi-core support. | Supported | Rebind markers, resulting TEID state, and compare or rollback evidence that the route remains auditable. | Medium. Useful for bounded recovery, but not a required milestone-1 proof path. | Broad re-anchoring parity |
...
- `supported` path elements are bounded reviewable support claims for maintenance or recovery without becoming part of the required pass path
