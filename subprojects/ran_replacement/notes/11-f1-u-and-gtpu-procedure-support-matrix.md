...
| `F1-U` stale-forwarding cleanup review | Confirm a failed or rolled-back user-plane attempt did not leave half-open forwarding state behind before another session attempt on the same UE lane. | Optional | Compare-report and rollback-evidence fields that name forwarding cleanup state and whether the next attempt stays bounded. | High. Half-open forwarding must stay explicit until the rollback target or clean slate is confirmed. | Multi-UE drain coordination, topology-scale traffic moves |
...
| `GTP-U` session-to-tunnel association | Keep the declared UE session, forwarding state, and tunnel state aligned. | Required | Session snapshot, health checks, and evidence bundle. | Rollback must preserve a deterministic session state transition that can be audited. | Session mobility, handover, roaming, cross-lab portability |
...
| `GTP-U` tunnel release and cleanup | Remove or restore tunnel state safely after failure, retry, or rollback. | Required | Release markers, stale-tunnel cleanup summary, and post-rollback tunnel state evidence. | High. Stale tunnel state is an explicit rollback risk and must be resolved before another session attempt. | Broad tunnel lifecycle parity beyond the declared lane |
| `GTP-U` bounded multi-session recovery review | Explain whether a follow-up session attempt on the same declared UE remains safe after tunnel cleanup, without claiming concurrent multi-session parity. | Optional | Compare-report and rollback evidence that name the active session, cleanup state, and next-attempt safety. | Medium. Keeps recovery review explicit before another session attempt on the same UE. | Concurrent multi-session throughput claims, multi-UE parity, roaming, mobility |
...
5. restore a clean prior state when cutover fails, including stale-tunnel cleanup before another declared session attempt.
...
- bounded multi-session wording means same-UE recovery review only; it is not a claim of concurrent session, multi-UE, or topology-scale parity
