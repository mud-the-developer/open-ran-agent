...
| `F1-C` UE context modification | Adjust the declared UE context without widening scope beyond the single-UE lane. | Supported | Modification markers, delta summary, and compare or rollback evidence that the post-change state still matches the target profile. | Medium. Useful when attach or rollback-safe cleanup needs a bounded context update. |
| `F1-C` reset-driven recovery | Provide a bounded escape hatch when the control-plane relationship is stale or half-open. | Supported | Explicit reset intent, post-reset peer state, and cleanup evidence that shows whether the lane returned to a known-safe state. | High. Must stay explicit because it is a recovery tool, not a happy-path milestone gate. |
...
| `E1AP` bearer context modification | Adjust the declared bearer state without widening scope beyond the single-session lane. | Supported | Modification markers, resulting bearer state, and compare or rollback evidence that the target session remains auditable. | Medium. Useful when the lane must explain a bounded bearer adjustment without claiming broad parity. |
...
- `supported` procedures are bounded reviewable support claims for recovery or maintenance without becoming part of the required pass path
