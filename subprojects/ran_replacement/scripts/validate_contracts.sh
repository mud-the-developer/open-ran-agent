...
echo "[1/5] Parsing replacement JSON fixtures"
for file in \
...
echo "[2/5] Validating ranctl replacement request fixtures"
...
echo "[3/5] Validating ranctl replacement status fixtures"
npx --yes ajv-cli validate \
  --spec=draft2020 \
  --strict=false \
  --validate-formats=false \
