...
7. Or edit:
   ...
   - `/etc/open-ran-agent/requests/plan-gnb-bringup.json`
   - `/etc/open-ran-agent/requests/verify-attach-ping.json`
   - `/etc/open-ran-agent/requests/rollback-gnb-cutover.json`
...
2. Or run `bin/ran-deploy-wizard --defaults --skip-install --target-host <host>` or use Deploy Studio.
   ...
   - review the generated replacement request bundle under `artifacts/deploy_preview/etc/requests/*`
...
6. Use `RAN_REMOTE_APPLY=1 bin/ran-remote-ranctl <host> <command> <request-file>` to drive remote `ranctl` across the declared lane:
   - `precheck` with `precheck-target-host.json`
   - `plan` and `apply` with `plan-gnb-bringup.json`
   - `verify` and `capture-artifacts` with `verify-attach-ping.json`
   - `rollback` with `rollback-gnb-cutover.json`
   Each run captures stdout locally under `artifacts/remote_runs/*` and auto-fetches matching remote evidence into `artifacts/remote_runs/*/fetch`.
...
- exporting quickstart lifecycle helpers for the full remote `ranctl` proof loop
...
- mirroring target-host artifacts, replacement proof artifacts, runtime evidence, and config snapshots back to the packaging host
